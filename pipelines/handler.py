...
class GeometryHandler(BaseHandler):
    ...
    def addObject(self, geo):
        """Registers a object derived from `BaseObject`. Must implement the methods of BaseObject."""
    ...
    def addGeo(self, geo):
        """Registers a `Geometry` object and sets its camera position"""
    ...
    def resetView(self):
        ...
        if self.cameraControl:
            self.cameraObject.setOrientation(
                quaternionFromEulerDeg(*self.initialCamRotation))
            self.objects.resetOrientation()
            cameraOffset = -Vector3(*self.initialCamPosition)
        else:
            self.cameraObject.resetOrientation()
            cameraRotation = quaternionFromEulerDeg(
                *self.initialCamRotation).conjugated()
    ...
    def toggleView(self):
        ...
        if self.cameraControl:
            self.cameraObject.setOrientation(
                self.objects.getOrientation().conjugated())
            self.objects.resetOrientation()
        else:
            self.objects.setOrientation(
                self.cameraObject.getOrientation().conjugated())
            self.cameraObject.resetOrientation()
